...
|----------|--------------|
| [Quality](.github/workflows/quality.yml) | CodeQL, dependency review and full npm/NuGet audits, CLI/Site lint and format checks, coverage and mutation thresholds, workflow lint, and a single aggregate quality gate. |
| [Mutation testing](.github/workflows/mutation.yml) | Weekly/manual Stryker runs for CLI projects and the site, with HTML/JSON reports. See [local commands and score policy](docs/mutation-testing.md). |
