...
Run `./scripts/validation/Run-SqlValidation.ps1` from PowerShell with Docker running and .NET 10 installed. The harness uses `mcr.microsoft.com/mssql/server:2022-latest` and localhost port 15439. The completed run checked 22 catalog queries, recreated eight definitions, and passed 11 source-versus-restored metadata comparisons.
