...
The initial Windows x64 run measured roughly 1 MiB of retained analysis growth at both synthetic sizes. It also exposed substantial Oracle parser allocation churn: about 4 GB allocated for a 16-object catalog and 16.7 GB for 64 objects, despite much lower retained memory. The initial Oracle budgets honestly accommodate that current behavior with headroom; they are regression ceilings, not a claim that this allocation rate is desirable. Optimizing it is a separate performance task. Limits should be tightened as measured behavior improves.
