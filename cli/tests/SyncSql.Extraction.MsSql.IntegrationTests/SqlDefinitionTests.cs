...
public sealed class SqlDefinitionTests(ITestOutputHelper output)
{
    [SqlIntegrationFact]
    [Trait("Category", "Integration")]
    public async Task Extracted_definitions_recreate_source_metadata()
    {
        string password = Environment.GetEnvironmentVariable("SYNCSQL_VALIDATION_PASSWORD") ?? throw new InvalidOperationException("SYNCSQL_VALIDATION_PASSWORD is required. Use Run-SqlValidation.ps1.");
        int port = int.Parse(Environment.GetEnvironmentVariable("SYNCSQL_VALIDATION_PORT") ?? throw new InvalidOperationException("SYNCSQL_VALIDATION_PORT is required. Use Run-SqlValidation.ps1."), System.Globalization.CultureInfo.InvariantCulture);
        string Connection(string database) => new SqlConnectionStringBuilder
        {
            ...
            ConnectTimeout = 3,
            // Retry real connections during container startup instead of cached pool failures.
            Pooling = false,
        }.ConnectionString;
        ...
        for (int attempt = 0; ; attempt++)
        {
            ...
            catch (SqlException) when (attempt < 120) { await Task.Delay(1000); }
        }
        ...
        foreach (FieldInfo field in queries.GetFields(BindingFlags.Public | BindingFlags.Static).Where(field => field.IsLiteral && field.FieldType == typeof(string)))
        {
            ...
            output.WriteLine($"Query bound: {field.Name}");
        }
        ServerConfig server = new()
        {
            ...
            TrustServerCertificate = true,
            Databases = new NameFilter { Include = ["^FixtureSource$"] },
            Schemas = new NameFilter { Include = ["^sales$"] },
        ...
        Assert.Equal(8, outcome.Objects.Count);
        ...
        foreach (ExtractedObject obj in outcome.Objects.OrderBy(Order))
        {
            ...
            output.WriteLine($"Created from export: {obj.Type}/{obj.Name}");
        }
        foreach (ExtractedObject obj in outcome.Objects.OrderBy(Order))
        {
            foreach (ExtractedSection section in obj.Sections)
            {
                await Execute("FixtureRestore", section.Content);
            }
        }
        string[] comparisons =
        [
        ...
        foreach (string comparison in comparisons)
        {
            ...
            Assert.True(Equals(source, restored), $"Restore mismatch: {comparison}: {source} != {restored}");
        }
        output.WriteLine($"SQL integration passed: {outcome.Objects.Count} definitions and {comparisons.Length} catalog comparisons.");
    }
}

internal sealed class ValidationLogger : ILogger<MsSqlObjectExtractor>
{
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        {
            throw new Exception(formatter(state, exception), exception);
        }
    }
}
