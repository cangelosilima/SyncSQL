...
pipeline's responsibility - see `.gitlab/ci/sync.yml`'s
...
result) is the calling pipeline's job - see `.gitlab/ci/sync.yml`.
...
different servers never collide. See `.gitlab/ci/extract.yml`'s
...
- **`git`**: where extracted objects get pushed - `remoteUrl`, `branch`
  ...
  directly (see `.gitlab/ci/sync.yml`, which reads it via `jq`);
...
in the reference `.gitlab/ci/sync.yml`) never passes through `syncsql` at
