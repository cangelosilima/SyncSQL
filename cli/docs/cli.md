...
| `--output-log`             | none                                | Append plain UTF-8 run logs to a file, alongside terminal output. Accepts relative or absolute paths (for example, `./logs/extraction.log`); creates missing parent directories. Includes warnings, errors, and the final summary. An invalid or unwritable path stops extraction before it starts. |
