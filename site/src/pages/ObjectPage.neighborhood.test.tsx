...
vi.mock('../lib/CatalogContext', () => ({
  useCatalog: () => ({ index: buildIndex(makeCatalog({
    nodes: ['caller', 'focus', 'target'].map(id => makeNode({ id })).concat([
...
it('includes callers and remote targets beyond links in the object graph', () => {
  render(<MemoryRouter initialEntries={['/object/focus']}><Routes><Route path="/object/*" element={<ObjectPage />} /></Routes></MemoryRouter>)
  fireEvent.click(screen.getByRole('tab', { name: 'Graph' }))
  expect(screen.getByTestId('graph').textContent?.split(',').sort()).toEqual(['caller', 'focus', 'incoming', 'outgoing', 'target'])
})
