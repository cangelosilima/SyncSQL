...
const object = makeNode({
  id: 'orders',
  ddl: 'CURRENT SQL',
  sections: [{ title: 'Constraints', content: 'CURRENT CONSTRAINT' }],
  history: [
...
vi.mock('../lib/CatalogContext', () => ({
  useCatalog: () => ({ index: buildIndex(makeCatalog({ nodes: [object] })) }),
}))
...
function open() {
  render(
    <MemoryRouter initialEntries={['/object/orders']}>
      <Routes>
...
describe('Object workspace state lifetime', () => {
  it('keeps historical SQL across workspace changes and returns appended sections only on latest', () => {
    ...
    expect(within(screen.getByRole('region', { name: 'Definition' })).getByText('OLD SQL')).toBeInTheDocument()
    expect(
      within(screen.getByRole('region', { name: 'Definition' })).queryByText('Constraints'),
  ...
  it('supports keyboard workspace navigation and exposes Relationships between Access and Metrics', () => {
    ...
    definition.focus()
    fireEvent.keyDown(definition, { key: 'ArrowRight' })
