...
const catalog = makeCatalog({ nodes: Array.from({ length: 12 }, (_, i) => makeNode({ id: `orders${i}`, metrics: [1000, 2000].map((rowCount, j) => ({ capturedAt: `2026-09-0${j + 1}`, rowCount, reservedKB: null, dataKB: null, indexKB: null, indexes: [], statistics: [] })) })), orphanedReferences: [{ from: 'orders0', name: 'Missing', schema: 'dbo', database: null, server: null }] })
...
it('summarizes all alerts between lineage and last change and links to investigation', () => {
  ...
  expect([...container.querySelectorAll('.quick-stat-label')].map(el => el.textContent)).toEqual(['Total objects', 'Commits mined', 'Lineage edges', 'Alerts', 'Last change'])
