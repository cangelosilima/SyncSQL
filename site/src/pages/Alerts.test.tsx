...
const catalog = makeCatalog({ nodes: [makeNode({ id: 'orders', metrics: [1000, 2000].map((rowCount, i) => ({ capturedAt: `2026-09-0${i + 1}`, rowCount, reservedKB: null, dataKB: null, indexKB: null, indexes: [], statistics: [] })) })], orphanedReferences: Array.from({ length: 105 }, (_, i) => ({ from: 'orders', name: `Missing${i}`, schema: 'dbo', database: null, server: null })) })
...
it('consolidates all findings, filters with shareable URL state, and preserves investigation links', () => {
  render(<MemoryRouter><Alerts /><Location /></MemoryRouter>)
...
it('restores category and search from a deep link', () => {
  render(<MemoryRouter initialEntries={['/alerts?category=orphans&q=Missing104']}><Alerts /></MemoryRouter>)
  expect(screen.getByRole('status')).toHaveTextContent('1 of 106 alerts match')
