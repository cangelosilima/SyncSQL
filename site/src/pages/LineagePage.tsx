...
export default function LineagePage() {
  ...
  const allNodes = baseIndex?.catalog.nodes ?? []
  const search = useCatalogFilter(baseIndex, tokens)
  const filtered = search.nodes
  const hasGranteeFilter = tokens.some((token) => token.attribute === 'grantee')
  const currentFocus = focusStack[focusStack.length - 1]
  const { index, loading, error } = useCatalogSelection(
    baseIndex,
    currentFocus
      ? {
          focus: currentFocus,
          dependencies: dependencyHops,
          dependents: dependentHops,
          grantIds: hasGranteeFilter ? [...filtered.map((node) => node.id), currentFocus] : [currentFocus],
        }
      : {
          ids: filtered.length <= GRAPH_CAP ? filtered.map((node) => node.id) : [],
          grantIds: hasGranteeFilter ? filtered.map((node) => node.id) : [],
        },
  )
  useEffect(() => {
  ...
  const grantMatches = hasGranteeFilter
    ? filtered
        ...
        .map((summary) => {
          const node = index?.byId.get(summary.id) ?? summary
          return { node, grants: matchingGrants(node, tokens) }
        })
    : []
  ...
  return (
    <div className="page page--wide lineage-page">
      <CatalogLoadStatus loading={loading || search.loading} error={error ?? search.error} />
      ...
      {!(loading || error || search.loading || search.error) && (
        <>
          {hasGranteeFilter && (
            <section aria-label="Matching permissions">
              <p className="muted">
                {totalGrants} grant{totalGrants === 1 ? '' : 's'} across {grantMatches.length} object
                {grantMatches.length === 1 ? '' : 's'} matching all filters. Recorded permissions include GRANT and
                DENY; connecting graph objects do not imply access.
              </p>
              ...
              {currentFocus && (
                <details className="lineage-view-options" open>
                  ...
                  <p className="muted">
                    ...
                    direction to 0 to hide it. Grouping combines intermediate objects of the same type and hop, keeping
                    the focus and outer objects visible.
                  </p>
                </details>
              )}
              <InspectorPanel>
