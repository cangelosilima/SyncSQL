...
it('lets visitors investigate each of the 48 workload users using recorded grants', () => {
  render(<MemoryRouter><BenchmarkExample catalog={catalog} /></MemoryRouter>)
  ...
  expect(screen.getByRole('link', { name: 'View recorded access' })).toHaveAttribute('href', `/lineage?tab=access&grantee=${user}&exact=1`)
})
