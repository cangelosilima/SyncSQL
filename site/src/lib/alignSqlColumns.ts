...
const modifiers =
  /^(NOT|NULL|DEFAULT|IDENTITY|GENERATED|COLLATE|CONSTRAINT|PRIMARY|REFERENCES|CHECK|UNIQUE|SPARSE|ROWGUIDCOL|FILESTREAM|MASKED|ENCRYPTED|HIDDEN|VISIBLE|INVISIBLE|ENABLE|DISABLE|AS)$/i
...
export function alignSqlColumns(
  code: string,
  config: Pick<SqlFormatConfig, 'tabWidth' | 'columnSpacing'> = sqlFormatConfig,
): string {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (
      isWord(tokens[cursor], /^IF$/i) &&
      isWord(tokens[cursor + 1], /^NOT$/i) &&
      isWord(tokens[cursor + 2], /^EXISTS$/i)
    )
      cursor += 3
    ...
    const columns = rows.flatMap((row) => {
      ...
      if (parts.some((token) => token.kind === 'comment' || /[\r\n\t]/.test(token.text)) || isWord(parts[1], /^AS$/i))
        return []
      let modifier = parts.length
      let level = 0
      for (let j = 1; j < parts.length; j++) {
        if (level === 0 && isWord(parts[j], modifiers)) {
          modifier = j
          break
        }
      ...
      return [
        { row, name: parts[0].text, type: compact(parts.slice(1, modifier)), modifier: compact(parts.slice(modifier)) },
      ]
    })
