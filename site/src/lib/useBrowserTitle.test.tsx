...
const order = {
  ...
  server: 'SQLPROD01', database: 'SalesDb', schema: 'dbo', name: 'Orders 100%',
} as CatalogNode
...
const link = {
  ...
  server: 'SQLPROD01', database: '_ServerLevel', schema: null, name: 'REMOTE',
} as CatalogNode
...
describe('browser tab and history titles', () => {
  it.each([
    ['/', 'Overview'], ['/explorer', 'Explorer'], ['/lineage', 'Lineage'],
    ['/alerts', 'Alerts'], ['/ai', 'AI'], ['/history', 'History'],
    ['/Explorer/', 'Explorer'], ['/missing', 'Page not found'],
  ...
  it('distinguishes objects across servers and restores titles on Back and Forward', () => {
    render(<MemoryRouter initialEntries={['/explorer']}><Page /></MemoryRouter>)
  ...
  it('resolves an object title after the catalog loads', () => {
    const view = (loaded: boolean) => <MemoryRouter initialEntries={[objectUrl(order)]}><Page loaded={loaded} /></MemoryRouter>
  ...
  it('names server-level objects without exposing the database placeholder', () => {
    render(<MemoryRouter initialEntries={[objectUrl(link)]}><Page /></MemoryRouter>)
