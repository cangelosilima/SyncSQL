...
describe('directional expansion and connecting paths', () => {
  it.each(['LinkedServers', 'DatabaseLinks'])(
    'shows one hop beyond %s on both sides, without unrelated branches',
    (type) => {
      const linked = buildIndex(
        makeCatalog({
          nodes: ['root', 'caller', 'target', 'callerParent', 'targetChild', 'sibling']
            .map((id) => makeNode({ id }))
            .concat([makeNode({ id: 'incomingLink', type }), makeNode({ id: 'outgoingLink', type })]),
          edges: [
            makeEdge('callerParent', 'caller'),
            makeEdge('caller', 'incomingLink'),
            makeEdge('incomingLink', 'root'),
            makeEdge('root', 'outgoingLink'),
            makeEdge('outgoingLink', 'target'),
            makeEdge('target', 'targetChild'),
            makeEdge('incomingLink', 'sibling'),
            makeEdge('sibling', 'outgoingLink'),
          ],
      ...
      expect(getDirectionalNeighborhood(linked, 'root', 1, 1).sort()).toEqual([
        'caller',
        'incomingLink',
        'outgoingLink',
        'root',
      ...
      expect(getDirectionalNeighborhood(linked, 'callerParent', 2, 0).sort()).toEqual([
        'caller',
        'callerParent',
        'incomingLink',
        'root',
  ...
  it.each(['LinkedServers', 'DatabaseLinks'])(
    'preserves caller/target pairs across a shared %s at every hop limit',
    (type) => {
      const shared = buildIndex(
        makeCatalog({
          nodes: ['caller', 'target', 'otherCaller', 'otherTarget', 'unresolvedCaller']
            .map((id) => makeNode({ id }))
            .concat(makeNode({ id: 'link', type })),
          edges: [
            makeEdge('caller', 'link'),
            makeEdge('otherCaller', 'link'),
            makeEdge('unresolvedCaller', 'link'),
            makeEdge('link', 'target'),
            makeEdge('link', 'otherTarget'),
          ],
          linkedServerReferences: [
      ...
      for (const hops of [1, 2, 3, 20]) {
        ...
        expect(getDirectionalNeighborhood(shared, 'unresolvedCaller', hops, 0).sort()).toEqual([
          'link',
          'unresolvedCaller',
        ])
      }
  ...
  it.each(['outgoing', 'incoming'])('revisits a shared link through distinct %s object paths', (direction) => {
    ...
    const reference = (from: string, to: string) => ({
      linkedServer: 'link',
      schema: null,
      name: direction === 'outgoing' ? to : from,
      from: direction === 'outgoing' ? from : to,
      to: direction === 'outgoing' ? to : from,
    })
    const shared = buildIndex(
      makeCatalog({
        nodes: ['root', 'a', 'b', 'c', 'd', 'end']
          .map((id) => makeNode({ id }))
    ...
    const expand = (hops: number) =>
      getDirectionalNeighborhood(
        shared,
        'root',
        direction === 'outgoing' ? hops : 0,
        direction === 'incoming' ? hops : 0,
      ).sort()
    ...
    const legacy = buildIndex(
      makeCatalog({
        nodes: [
          makeNode({ id: 'caller' }),
          makeNode({ id: 'target' }),
          makeNode({ id: 'link', type: 'DatabaseLinks' }),
        ],
  ...
  it('keeps the benchmark ITEMS flow separate from P_CIRCLE even at twenty hops', () => {
    ...
    for (const hops of [1, 2, 3, 20]) {
      const names = getDirectionalNeighborhood(benchmark, root, 0, hops).map(
        (id) => benchmark.byId.get(id)!.qualifiedName,
      )
    ...
    const names = getDirectionalNeighborhood(benchmark, circle, 20, 20).map(
      (id) => benchmark.byId.get(id)!.qualifiedName,
    )
  ...
  const directed = buildIndex(
    makeCatalog({
      ...
      edges: [
        ...
        makeEdge('dep', 'far'),
        makeEdge('user', 'root'),
        makeEdge('caller', 'user'),
        makeEdge('sibling', 'dep'),
        makeEdge('far', 'root'),
      ],
    }),
  )
  ...
  it('retains filtered intermediate objects without unrelated branches', () => {
    expect(retainConnectingPaths(index, 'root', getNeighborhoodIds(index, 'root', 2), new Set(['far']))).toEqual([
      'root',
      'downstream',
      'far',
    ])
    expect(retainConnectingPaths(index, 'root', getNeighborhoodIds(index, 'root', 2), new Set(['island']))).toEqual([
      'root',
    ])
  })
