...
describe('catalog analytics', () => {
  ...
  it('orders timestamps by instant across different UTC offsets and limits rankings', () => {
    ...
    expect(getMostChanged(index)).toEqual([
      { node: earlier, value: 5 },
      { node: later, value: 2 },
    ])
  ...
  it('ranks tables by direct users then transitive users and ignores unused tables', () => {
    ...
    expect(
      getTopReferencedTables(index).map(({ node, directUsers, indirectUsers }) => [
        node.id,
        directUsers,
        indirectUsers,
      ]),
  ...
  it.each([
    ...
    [5, 10, 0.5],
    [20, 10, 1],
  ])('clamps intensity(%s, %s)', (value, max, expected) => {
  ...
  it.each([
    ...
    [86400, '2mo ago'],
    [1051200, '2y ago'],
  ])('formats %s minutes elapsed', (minutes, expected) => {
    vi.useFakeTimers()
