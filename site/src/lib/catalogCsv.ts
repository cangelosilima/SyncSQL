...
export function objectColumns(index: CatalogIndex): CsvColumn<CatalogNode>[] {
  return [
    ...
    { header: 'ColumnCount', value: (node) => node.columnCount ?? node.columns.length },
    { header: 'GrantCount', value: (node) => node.grantCount ?? node.grants.length },
    { header: 'DependsOnCount', value: (node) => node.dependsOnCount ?? index.outgoing.get(node.id)?.length ?? 0 },
    { header: 'UsedByCount', value: (node) => node.usedByCount ?? index.incoming.get(node.id)?.length ?? 0 },
