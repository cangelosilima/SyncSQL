...
beforeEach(() => {
  ...
  document.documentElement.style.cssText =
    '--surface:#fffdf8;--surface-alt:#eee7dc;--selected:#dcebe2;--text:#302d28;--border:#918576;--accent:#28665c;--text-muted:#696052;--font-body:Source Sans 3, sans-serif'
})
afterEach(() => {
  vi.restoreAllMocks()
  vi.unstubAllGlobals()
...
const nodes: Node[] = [
  {
    id: 'a',
    data: { label: 'SCHEMA.A_VERY_LONG_IDENTIFIER_WITH_NO_TRUNCATION<&>' },
    position: { x: 0, y: 0 },
    measured: { width: 220, height: 74 },
    className: 'lineage-node--focus',
    style: { background: 'var(--selected)', border: '2px solid #ff9900' },
  },
  {
    id: 'b',
    data: { label: '8 grouped Tables' },
  ...
  const svg = buildLineageGraphSvg(nodes, [
    {
      ...
      target: 'b',
      label: 'ID, AMOUNT',
      style: { stroke: '#28665c', strokeWidth: 1.5, strokeDasharray: '4 3' },
    },
  ])
...
it('fits reverse Bezier control points and skips hidden nodes', () => {
  const svg = buildLineageGraphSvg(
    [...nodes, { ...nodes[0], id: 'hidden', hidden: true, position: { x: 10000, y: 10000 } }],
  ...
  for (let i = 0; i < points.length; i += 2) {
    expect(points[i]).toBeGreaterThan(x)
    expect(points[i]).toBeLessThan(x + width)
    expect(points[i + 1]).toBeGreaterThan(y)
    expect(points[i + 1]).toBeLessThan(y + height)
  }
...
it.each([220, 1380])('always places a complete legend below the graph at width %s', (width) => {
  ...
  for (const label of [
    ...
    'recorded column references',
    'dynamic SQL reference (weaker evidence)',
    'Current focus',
    'grouped objects',
    'not complete column lineage',
    'Packages',
    'Tables',
  ])
