...
const grantKey = (object, user, permission, state, column) =>
  JSON.stringify([object, user, permission, state, column ?? null])
...
export function validateExample(catalog, contract) {
  assert.deepEqual(
    sorted(catalog.nodes.map((n) => n.id)),
    sorted(contract.nodes.map((n) => n.id)),
    'Catalog identities differ',
  )
  ...
  for (const expected of contract.nodes) {
    ...
    for (const match of actual.ddl.matchAll(
      /(?:IDENTIFIED\s+BY|@rmtpassword\s*=)\s*(?:N)?(?:'([^']*)'|"([^"]*)"|([^\s;]+))/gi,
    )) {
      assert.match(match[1] ?? match[2] ?? match[3], /^[#*]+$/, `${expected.id}: unmasked password`)
    }
  }
  const expectedEdges = contract.dependencies.flatMap((d) =>
    d.via ? [edgeKey(d.from, d.via), edgeKey(d.via, d.to)] : [edgeKey(d.from, d.to)],
  )
  assert.deepEqual(
    unique(catalog.edges.map((e) => edgeKey(e.from, e.to))),
    unique(expectedEdges),
    'Lineage edges differ',
  )
  assert.deepEqual(
    sorted(catalog.linkedServerReferences.map((r) => `${r.from}|${r.linkedServer}|${r.to}|${!!r.dynamic}`)),
    sorted(contract.dependencies.filter((d) => d.via).map((d) => `${d.from}|${d.via}|${d.to}|${!!d.dynamic}`)),
    'Remote attribution differs',
  )
  assert.deepEqual(catalog.orphanedReferences ?? [], [], 'Unresolved references')
  const users = new Set(contract.users.map((u) => u.name))
  assert.deepEqual(
    sorted(
      catalog.nodes.flatMap((n) =>
        n.grants
          .filter((g) => users.has(g.grantee))
          .map((g) => grantKey(n.id, g.grantee, g.permission, g.state, g.column)),
      ),
    ),
    sorted(contract.grants.map((g) => grantKey(g.object, g.user, g.permission, g.state, g.column))),
    'Workload grants differ',
  )
  ...
  for (const route of contract.paths) {
    ...
    for (let i = 1; i < route.nodes.length; i++)
      assert.ok(semanticEdges.has(edgeKey(route.nodes[i - 1], route.nodes[i])), route.name)
  }
  if (catalog.example) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (process.argv[2] === '--check') {
    ...
    console.log(
      `Verified benchmark example: ${catalog.nodes.length} objects, ${contract.users.length} users, ${contract.paths.length} paths.`,
    )
  } else {
    ...
    const example = prepareExample(
      await read(path.join(runRoot, 'extracted/catalog.json')),
      contract,
      await read(path.join(runRoot, 'benchmark.json')),
    )
