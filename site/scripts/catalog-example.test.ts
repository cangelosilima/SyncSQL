...
describe('published benchmark example', () => {
  ...
  it('rejects missing edges, lost permissions and misattributed remote paths', () => {
    for (const mutate of [
      ...
      (copy: typeof catalog) => {
        copy.nodes.find((n: { grants: unknown[] }) => n.grants.length).grants = []
      },
      (copy: typeof catalog) => {
        copy.linkedServerReferences[0].to = copy.linkedServerReferences[1].to + '-wrong'
      },
    ]) {
  ...
  it('requires a successful live gateway run and rejects unmasked passwords before publication', () => {
    expect(() => prepareExample(catalog, contract, { scenario: 'heterogeneous-lineage', passed: false })).toThrow(
      /must pass/,
    )
    expect(() =>
      prepareExample(catalog, contract, { scenario: 'heterogeneous-lineage', passed: true, gatewayRequested: false }),
    ).toThrow(/gateway/i)
    const copy = structuredClone(catalog)
